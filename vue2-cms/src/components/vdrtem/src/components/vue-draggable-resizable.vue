...
export default {
  ...
  methods: {
    ...
    moveHorizontally (val) {
      const [deltaX, y] = snapToGrid(this.grid, val, this.top, this.scale)
      this.myy = y
    ...
    moveVertically (val) {
      const [x, deltaY] = snapToGrid(this.grid, this.left, val, this.scale);
      this.myx = x
    ...
    changeWidth (val) {
      const [newWidth, y] = snapToGrid(this.grid, val, 0, this.scale)
      this.myy = y
    ...
    changeHeight (val) {
      const [x, newHeight] = snapToGrid(this.grid, 0, val, this.scale)
      this.myx = x
